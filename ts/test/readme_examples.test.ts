...
const TEST_SEED = {"entity":{"author":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+RealgazetaSDK\b/.test(code) ||
    /\bRealgazetaSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RealgazetaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RealgazetaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

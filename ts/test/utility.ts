...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.REALGAZETA_TEST_LIVE ||
    'TRUE' === process.env.REALGAZETA_TEST_OVERRIDE
  ) {
  ...
  m.REALGAZETA_TEST_EXPLAIN = process.env.REALGAZETA_TEST_EXPLAIN || m.REALGAZETA_TEST_EXPLAIN

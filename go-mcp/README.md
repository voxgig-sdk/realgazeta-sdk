...
[MCP](https://modelcontextprotocol.io) server exposing the Realgazeta SDK as
two agent tools — `realgazeta_list` and `realgazeta_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/realgazeta-mcp)
make build

# 2. Provide credentials via the environment
export REALGAZETA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user realgazeta \
  -- /absolute/path/to/realgazeta-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./realgazeta-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// realgazeta_list: first page of records
{ "entity": "author" }
{ "entity": "author", "query": { } }

// realgazeta_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/realgazeta-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user realgazeta \
     -- "$PWD"/dist/*/realgazeta-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `realgazeta_list` and `realgazeta_load` tools now appear
   in new sessions. Ask the agent to *"list author using realgazeta"*
   and it calls `realgazeta_list` with `{"entity":"author"}`.
...
```sh
export REALGAZETA_APIKEY=sk_live_xxx            # API key
export REALGAZETA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `realgazeta_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `realgazeta_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REALGAZETA_APIKEY` | API key sent with every request. |
| `REALGAZETA_BASE` | Optional override of the API base URL. |
...
```sh
./realgazeta-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"realgazeta_load","arguments":{"entity":"author","query":{"id":1}}}}'
```

...
local AuthorEntity = {}
AuthorEntity.__index = AuthorEntity


function AuthorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthorEntity)
  self._name = "author"
  self._client = client
...
function AuthorEntity:get_name()
  return self._name
end


function AuthorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthorEntity.new(self._client, opts)
end
...
function AuthorEntity:mark_deleted()
  self._deleted = true
end


function AuthorEntity:deleted()
  return true == self._deleted
end


function AuthorEntity:data_set(args)
  if args ~= nil then
...
function AuthorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthorEntity:match_set(args)
  if args ~= nil then
...
function AuthorEntity:match_get()
...
function AuthorEntity:stream(action, args, callopts)
...
---@param reqmatch AuthorLoadMatch
---@param ctrl? table
---@return Author
---@return string? err
function AuthorEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AuthorEntity:_run_op(ctx, post_done)
  local utility = self._utility

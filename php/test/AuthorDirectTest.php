...
{
    public function test_direct_load_author(): void
    {
        $setup = author_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-author", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function author_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new RealgazetaSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,

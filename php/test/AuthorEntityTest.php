...
class AuthorEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Author(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = author_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "author." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set REALGAZETA_TEST_AUTHOR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $author_ref01_data = null;
        if (count($author_ref01_data_raw) > 0) {
...
function author_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/author/AuthorTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["author01", "author02", "author03", "slug01", "slug02", "slug03"] as $k) {
    ...
    $entid_env_raw = getenv("REALGAZETA_TEST_AUTHOR_ENTID");
    ...
    if ($env["REALGAZETA_TEST_LIVE"] === "TRUE") {
        ...
        $client = new RealgazetaSDK(Helpers::to_map($merged_opts));
    }

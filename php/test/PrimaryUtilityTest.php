...
// Realgazeta SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        RealgazetaSDK $client,
        RealgazetaUtility $utility
    ): RealgazetaContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new RealgazetaContext($ctxmap, null);
    ...
    private static function fixctx(RealgazetaContext $ctx, RealgazetaSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?RealgazetaError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RealgazetaError($code, $msg);
    }
    ...
    {
        $client = RealgazetaSDK::test(null, null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new RealgazetaResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new RealgazetaBaseFeature();
    ...
    {
        $hook_client = RealgazetaSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends RealgazetaBaseFeature {
            public $hook_fn = null;
            public function TestHook(RealgazetaContext $ctx): void
            {
    ...
    {
        $init_client = RealgazetaSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends RealgazetaBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new RealgazetaSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new RealgazetaSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = RealgazetaSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = RealgazetaSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof RealgazetaError) ? $result->err->msg : (string)$result->err;

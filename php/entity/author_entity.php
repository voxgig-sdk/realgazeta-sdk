...
class AuthorEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AuthorEntity($this->_client, $opts);
    }

    /**
     * @param Author|array $args Author data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RealgazetaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Author|array The current Author data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Author fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RealgazetaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Author fields).
     */
     ...
     *
     * @param AuthorLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AuthorLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Author|array The loaded Author as an assoc-array at the
     *   SDK boundary; throws RealgazetaError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RealgazetaHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

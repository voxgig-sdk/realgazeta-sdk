...
class RealgazetaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RealgazetaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RealgazetaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RealgazetaTestRunner.getenv("REALGAZETA_TEST_LIVE")
        override = RealgazetaTestRunner.getenv("REALGAZETA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RealgazetaTestRunner.getenv(key)
        ...
        explain = RealgazetaTestRunner.getenv("REALGAZETA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["REALGAZETA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RealgazetaTestRunner._test_control is not None:
            return RealgazetaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RealgazetaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RealgazetaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RealgazetaTestRunner.load_test_control()
...
def load_env_local():
    RealgazetaTestRunner.load_env_local()


def env_override(m):
    return RealgazetaTestRunner.env_override(m)


def entity_data(v):
    return RealgazetaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RealgazetaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RealgazetaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RealgazetaTestRunner.load_test_control()


def live_delay_ms():
    return RealgazetaTestRunner.live_delay_ms()

...
boru-driven command-line client **and** interactive REPL for the Realgazeta
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/realgazeta-cli)
make build
...
./realgazeta-cli load 1 author            # {id:1} shorthand
./realgazeta-cli load '{id:1}' author       # explicit match map

# 5. Override the API base URL for a single call
REALGAZETA_BASE=https://api.example.com ./realgazeta-cli load 1 author

# 6. No arguments -> interactive REPL
./realgazeta-cli
realgazeta> load 1 author
realgazeta> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/realgazeta-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/realgazeta-cli load 1 author
   ```
...
```sh
./realgazeta-cli load 1 author          # scalar shorthand for {id:1}
./realgazeta-cli load '{id:1}' author     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `realgazeta>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `author`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/realgazeta-cli`. |
...
(`./realgazeta-cli <expr>`) and interactively (the REPL), and expressions compose the

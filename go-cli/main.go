// Command Realgazeta-cli is a CLI + REPL for the Realgazeta SDK,
...
const prompt = "realgazeta"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "author"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REALGAZETA_APIKEY (api key), REALGAZETA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REALGAZETA_APIKEY carries the API key and
	// REALGAZETA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REALGAZETA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REALGAZETA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRealgazetaSDK(opts)

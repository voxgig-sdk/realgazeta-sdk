...
module RealgazetaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RealgazetaResponse.new({})
    ...
    if fetch_err
      ...
      response = RealgazetaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RealgazetaResponse.new(fetched)
    else

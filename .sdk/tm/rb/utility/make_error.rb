...
module RealgazetaUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = RealgazetaContext.new({}, nil)
    end
    op = ctx.op || RealgazetaOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || RealgazetaResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(RealgazetaError) ? err.msg : err.to_s
    ...
    sdk_err = RealgazetaError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(RealgazetaError)

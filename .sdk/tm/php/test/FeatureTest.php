...
class FtCtrl extends RealgazetaControl
{
...
class FtClient
{
    ...
    private RealgazetaUtility $utility;

    public function __construct(RealgazetaUtility $utility, array $options)
    {
    ...
    public function get_utility(): RealgazetaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RealgazetaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new RealgazetaUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (RealgazetaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RealgazetaConfig::make_feature($name);
    ...
    private static function buildUrl(RealgazetaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RealgazetaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RealgazetaResponse($fetched);
        }
    ...
    private function fail(RealgazetaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RealgazetaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
class RealgazetaMakeError
{
    public static function call(?RealgazetaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new RealgazetaContext([], null);
        }
        $op = $ctx->op ?? new RealgazetaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new RealgazetaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof RealgazetaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new RealgazetaError('', $msg, $ctx);
        ...
        if ($err instanceof RealgazetaError) {

...
require_relative 'Realgazeta_types'
...
class RealgazetaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RealgazetaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RealgazetaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RealgazetaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RealgazetaUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RealgazetaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RealgazetaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RealgazetaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RealgazetaError.new(
        "#{op}_allow",
        "RealgazetaSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RealgazetaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RealgazetaError.new(
        "graphql_error", "RealgazetaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

...
class AuthorEntity
  ...
  def make
    opts = @_entopts.dup
    AuthorEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RealgazetaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Author, Hash] the current Author data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RealgazetaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Author fields)
  def match_get
  ...
  #
  # @param reqmatch [AuthorLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Author.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Author, Hash] the loaded Author; raises RealgazetaError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RealgazetaHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
